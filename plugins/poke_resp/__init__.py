...
async def handle_function(event: NoticeEvent):
    ...
    if isinstance(event, PokeNotifyEvent):
        if event.target_id == event.self_id:
            ...
            msg = ""
            if rd <= 10:
                msg = Message('[CQ:image,file=file:///W:/soft/web_svr/testpilot_qqbot/images/poke/poke_'+str(rd)+'.jpg,sub_type=1]')
            elif rd == 11:
                msg = Message('[CQ:image,file=file:///W:/soft/web_svr/testpilot_qqbot/images/poke/poke_11.gif,sub_type=1]')
            elif 11 < rd < 14:
                msg = "干嘛！"
            elif 13 < rd < 16:
                msg = "不可以戳我哟！"
